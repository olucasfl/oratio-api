...
| **Entrar com Google** (login social; `LinkedAccount`, `User.password` opcional) | `docs/specs/login-google.md` · `docs/tasks/login-google-plan.md` · `docs/tasks/login-google-todo.md` | 🚧 **Ativo — Fase A (backend) na `develop`**. Plano-mestre cross-repo; frontend tem ponteiro em `oratio/docs/tasks/login-google-todo.md`. `db push` (local dev + produção) e cliente OAuth no Google Cloud Console são pendências humanas — `docs/specs/INDEX.md`. Decisão pendente: `deleteAccount` de conta só-Google. |
