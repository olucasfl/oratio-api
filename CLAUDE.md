...
| **Entrar com Google** (login social; `LinkedAccount`, `User.password` opcional) | `docs/specs/login-google.md` · `docs/tasks/login-google-plan.md` · `docs/tasks/login-google-todo.md` | 🚧 **Fases A, B, C na `develop`** (backend: `POST /auth/google`, A8 `deleteAccount` re-auth Google, C1 `hasPassword`; frontend: botão GIS, UI "definir senha"). **Fase D** (CSP do GIS) na branch `oratio:feat/login-google-fase-d`. Plano-mestre cross-repo; frontend tem ponteiro em `oratio/docs/tasks/login-google-todo.md`. **Fase E** (mensageria + `isNewUser`/`googleLinkedNow` + correções de frontend) aprovada, branch `docs/login-google-fase-e`. Pendências **humanas**: merge da Fase D, env vars (Vercel/Render), cliente OAuth no Google Cloud Console, teste manual + verificação pós-deploy da CSP — `docs/specs/INDEX.md`. **`db push` de produção FEITO** (2026-09-09, Supabase). |
