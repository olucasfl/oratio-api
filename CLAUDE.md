...
| **Boas-vindas** (guia de primeira entrada) | `docs/specs/boas-vindas.md` | 🚧 **Na `develop`** (2026-09-10) — **`main` não tem nada**. `User.welcomeSeenAt DateTime?` + `showWelcome` no `GET /users/me` + `POST /users/me/welcome-seen` (idempotente). **`db push` + backfill de produção PENDENTES DE EXECUÇÃO HUMANA** (`prisma/db-scripts/2026-09-10-boas-vindas.sql`) — o código já está na `develop` e dá 500 no `GET /users/me` até a coluna existir. `ARCHITECTURE.md` §4/§7. |
