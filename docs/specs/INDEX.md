...
| Admin: método de entrada | `specs/admin-provedor.md` | *(a criar)* | *(a criar)* | `oratio/docs/specs/admin-provedor.md` (ponteiro, a criar) | 📝 **rascunho** (branch `docs/spec-admin-provedor`) — ver/filtrar Oratio × Google × ambos no painel admin. Só leitura: `hasPassword` + `authProviders` no `GET /users/admin/users`, filtro `provider`, ícones no frontend. **Sem schema, sem `db push`.** |
