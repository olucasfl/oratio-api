...
| Boas-vindas (guia de primeira entrada) | `specs/boas-vindas.md` | *(a criar)* | *(a criar)* | `oratio/docs/specs/boas-vindas.md` (ponteiro, a criar) | ✅ **aprovada** (branch `docs/spec-boas-vindas`) — guia de 3 páginas na 1ª entrada (senha e Google), 1x só. Backend: `User.welcomeSeenAt` (paralelo a `voxOnboardingSeenAt`) + `showWelcome` no `GET /users/me` + `POST /users/me/welcome-seen` + **backfill** `welcomeSeenAt=now()` em toda a tabela. `db push` + backfill juntam com o do login-google — **um push**. Falta plano/checklist. |
...
- **boas-vindas** — **um único `npx prisma db push` de produção** cobre login-google (Fase D)
  **e** a coluna `User.welcomeSeenAt`. O **backfill** `UPDATE "User" SET welcomeSeenAt = now()`
  (escreve em toda a tabela) é um passo SQL manual **na mesma janela**, do mesmo `.sql`. Não
  rodar dois pushes. Script `prisma/db-scripts/2026-09-09-boas-vindas.sql` com passos numerados
  (contagem antes · ALTER · UPDATE · conferência) — gerado por `/db-change` na implementação.
